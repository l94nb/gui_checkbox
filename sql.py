...
def insert_data_to_db(collector_id: int, collected_data: float, collected_time: str):
    ...
    if os.path.isfile('data.txt'):
        with open('data.txt', 'r') as f:
            for line in f:
                collector_id, collected_data, collected_time = line.strip().split('\t')
    ...
    # 将数据直接写进数据库
    cursor.execute("INSERT INTO data_table (采集器id, 设备数据, 采集时间) VALUES (%s, %s, %s)",(collector_id, collected_data, collected_time))
    conn.commit()
